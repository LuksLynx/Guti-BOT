...
module.exports = {
	...
	async execute(message, args) {
		...
		if (!question) {
			return message.reply('Pergunta algo né imbecil.');
		}
		if (question.length > 75) {
			return message.reply('Tá escrevendo um livro?');
		}
		if (!question.endsWith('?')) {
			return message.reply('Isso não é uma pergunta.');
		}
		if (!question.match(regex)) {
			return;
		}

		let randAnswer = await random(0, answers.length);
