...
module.exports = {
    ...
    async execute(message, args) {
        ...
        if(mensagem.some(element => element ==''))
            return message.channel.send('Não deixe coisas vazias >:(');
        if (mensagem.length < 3)
            return message.channel.send('É preciso pelo menos 2 alternativas');
        ...
        const embedMessage = await message.channel.send(pollEmbed);
